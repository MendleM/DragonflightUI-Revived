...
local function AddFlyoutTable(optionTable, sub)
    local extraOptions = {
        ...
        inventoryCount = {
            type = 'editbox',
            name = L["FlyoutInventoryCount"],
            desc = L["FlyoutInventoryCountDesc"] .. getDefaultStr('inventoryCount', sub),
            Validate = function()
                return true
            end,
            order = 10.4,
...
function Module:RegisterOptionScreens()
    for i = 1, numCustomButtons do
        ...
        DF.ConfigModule:RegisterSettingsData(n, 'flyout', {
            options = customOptionsTable[i],
            sortComparator = customOptionsTable[i].sortComparator,
            default = function()
                setDefaultSubValues(customOptionsTable[i].sub)
            end
        })
    end
end
...
function Module:AddEditMode()
    ...
    for i = 1, numCustomButtons do
        ...
        btn.DFEditModeSelection:SetGetLabelTextFunction(function()
            return customOptionsTable[i].name
        end)
