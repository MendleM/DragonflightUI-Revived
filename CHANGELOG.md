...
**Highlights** — party frames survive a group member levelling up, or an invite, in combat · raid-style party frames take effect on the next reload and ask for one when switched · no more `combat ended - finishing setup` after a group invite, and no frame drop with it · leftover `DragonflightUI_Layout` renamed and reused, or removed · party members get their class colour as soon as the game knows it · mouse clicks no longer blocked when disabling the Action Bar module
### Action Bars
